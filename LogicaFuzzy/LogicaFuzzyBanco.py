...
class SistemaAnaliseRiscoFuzzy:
    ...
    # se o histórico de crédito é bom, a renda é baixa, e a dívida é moderada, o risco é moderado
        ...
        rule14 = ctrl.Rule(self.historico_credito['excelente'] & self.renda_mensal['baixa'] & self.divida_atual['moderada'], self.risco['medio'])

        rule15 = ctrl.Rule(self.historico_credito['regular'] & self.renda_mensal['media'] & self.divida_atual['baixa'], self.risco['baixo'])
        
        rule16 = ctrl.Rule(self.historico_credito['ruim'] & self.renda_mensal['alta'] & self.divida_atual['moderada'], self.risco['alto'])
        
        rule17 = ctrl.Rule(self.historico_credito['regular'] & self.renda_mensal['alta'] & self.divida_atual['alta'], self.risco['medio'])

        rule17 = ctrl.Rule(self.historico_credito['ruim'] & self.divida_atual['moderada'], self.risco['baixo'])

    # atribuir as regras
        self.regras = [rule1, rule2, rule3, rule4, rule5, rule6, rule7, rule8, rule9, rule10, rule11, rule12, rule13, rule14, rule15, rule16, rule17]
    ...
    def avaliar_risco(self, historico_credito_val, renda_mensal_val, divida_atual_val):
        ...
        #função de pertinência da saída
        self.risco.view(sim=self.simulador)
        plt.show()
